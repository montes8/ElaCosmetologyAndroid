...
class CameraController (private val context: ComponentActivity, private val namePath :String, private val listener: CameraControllerListener?){
    ...
    fun onActivityResult(resultCode: Int, data: Intent?) {
        if(resultCode == Activity.RESULT_OK) {
            ...
            val storageDir = File(context.getExternalFilesDir(Environment.DIRECTORY_PICTURES), "" + namePath)
            if(data == null) {
            ...
            for (i in files.indices) {
                ...
                if(imgFile.name == pictureNameTemp && imgFile.exists()) {
                    ...
                    try {
                        ...
                        listener?.onGetImageCameraCompleted(path,rotateIfNeeded(imgGallery,Uri.fromFile(file)))
                    } catch (e: IOException) {
                        e.printStackTrace()
                    }
                }
